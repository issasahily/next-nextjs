...
async function handler(req, res) {
    if (req.method === "POST") {
        const data = req.body;
        const client = await external_mongodb_namespaceObject.MongoClient.connect("mongodb+srv://issa:issayoussef@cluster0.p9ekjvo.mongodb.net/meetup?retryWrites=true&w=majority");
        const db = client.db();
        ...
        client.close();
        res.status(201).json({
            message: "the meetup successfuly!"
        });
    }
}
